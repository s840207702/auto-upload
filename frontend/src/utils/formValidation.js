...
import { validateTitle, validateTopics, validateSingleTopic, getPlatformConfig } from './platformConfig'

function getSelectedPlatformKeys(formData) {
  if (Array.isArray(formData.selectedPlatformTypes) && formData.selectedPlatformTypes.length > 0) {
    return formData.selectedPlatformTypes
  }
  return formData.selectedPlatform ? [formData.selectedPlatform] : []
}

function getStrictestPlatformKey(platformKeys, limitName) {
  if (!platformKeys.length) return null
  return platformKeys
    .map(key => getPlatformConfig(key))
    .sort((a, b) => (a[limitName] || 999) - (b[limitName] || 999))[0]?.key
}

function getStrictestNonBiliPlatformKey(platformKeys, limitName) {
  return getStrictestPlatformKey(platformKeys.filter(key => Number(key) !== 5), limitName)
}
...
export function validatePublishForm(formData) {
  const errors = {}
  let isValid = true
  const platformKeys = getSelectedPlatformKeys(formData)
  ...
  // 公共作品文案可选，仅校验长度；B站稿件标题单独必填。
  const titlePlatformKey = getStrictestNonBiliPlatformKey(platformKeys, 'titleLimit') || formData.selectedPlatform
  if (titlePlatformKey) {
    const titleValidation = validateTitle(formData.title, titlePlatformKey, { required: false })
  ...
  if (platformKeys.some(key => Number(key) === 5)) {
    const biliTitleValidation = validateTitle(formData.biliTitle || '', 5)
    if (!biliTitleValidation.valid) {
      errors.biliTitle = '请填写B站稿件标题'
      isValid = false
    }
  }
  ...
  // 验证账号
  if (Array.isArray(formData.selectedPlatformTypes)) {
    if (!formData.selectedProfileName) {
      errors.profile = '请选择账号主体'
      isValid = false
    }
  } else if (!formData.selectedAccounts || formData.selectedAccounts.length === 0) {
    errors.accounts = '请选择发布账号'
    isValid = false
  }

  // 验证话题
  const topicPlatformKey = getStrictestPlatformKey(platformKeys, 'topicLimit') || formData.selectedPlatform
  const topicsValidation = validateTopics(formData.selectedTopics, topicPlatformKey)
  ...
  if (formData.scheduleEnabled) {
    ...
    if (formData.timeJitterMinutes === undefined || formData.timeJitterMinutes < 0 || formData.timeJitterMinutes > 120) {
      errors.timeJitterMinutes = '随机浮动需在0-120分钟之间'
      isValid = false
    }
  }
...
export function validateField(fieldName, value, context = {}) {
  switch (fieldName) {
    case 'title':
      return validateTitle(value, context.platformKey, { required: context.required !== false })
    ...
    case 'timeJitterMinutes':
      if (value === undefined || value < 0 || value > 120) {
        return { valid: false, message: '随机浮动需在0-120分钟之间' }
      }
      return { valid: true, message: '' }
